...
module.exports = function(config) {
  ...
  session.config = {
    ...
    /**
     * @cfg {String} cookieHeader='Set-Cookie' The Response Cookie Header
     */
    cookieHeader: 'Set-Cookie',
    cookieName: 'restifySessionid'
  };
  ...
  session.setSessionData = function(sid, data, req, res, next) {
    ...
    session.save(sid, data, function(err, status){
      if (!err) {
        ...
        res.setCookie(cfg.cookieName, sid, {
          path: '/',
          expires: (new Date(new Date().getTime() + cfg.ttl * 1000 ))
        });
      }
      next();
    });
  ...
  session.sessionManager = function (req, res, next) {
    ...
    /*if (!res.setCookie) {
      session.useCookieParse(req, res, next);
    }*/
    if (cfg.debug) {
      cfg.logger.log(session.name + ': request url: ' + req.url);
    }
    var reqSid = req.cookies[cfg.cookieName];
   ...
   * @return {undefined}
   */
  session.useCookieParse = function (req, res, next) {
    if (cfg.debug) {
      cfg.logger.log(session.name + ': use cookie parse ');
    }
